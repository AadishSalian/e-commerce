...
export default function HomeFurniturePage() {
  ...
  const filteredProducts = MOCK_PRODUCTS.filter(p => p.category === 'Home & Furniture')
  .sort((a, b) => {
      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="mb-16">
          ...
          <p className="text-text-muted text-lg max-w-2xl">
            Elevate your living space with our collection of minimalist furniture and home essentials.
          </p>
        </div>
                ...
                <Link href={`/products/${product.id}`} className="group block">
                  ...
                  <div className="w-full aspect-[4/5] bg-surface rounded-xl border border-transparent group-hover:border-border transition-colors duration-300 mb-6 flex flex-col p-6 relative overflow-hidden">
                    ...
                    <div className="flex-1 w-full bg-surface-hover rounded-lg flex items-center justify-center">
                      <span className="text-xs text-text-muted uppercase tracking-widest text-center px-4">{product.name}</span>
                    </div>
                  </div>
