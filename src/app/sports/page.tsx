...
export default function SportsPage() {
  ...
  const [activeType, setActiveType] = useState<string>('All Gear Types');
  const [activeEnvironment, setActiveEnvironment] = useState<string>('All Environments');

  const categories = ['All Activities', 'Training', 'Outdoor', 'Recovery', 'Travel'];
  const gearTypes = ['All Gear Types', 'Equipment', 'Apparel', 'Accessories'];
  const environments = ['All Environments', 'Indoor', 'Alpine', 'Studio', 'All-Weather'];
  ...
  const filteredProducts = useMemo(() => {
    return sportsProducts.filter(p => {
      ...
      const envMatch = activeEnvironment === 'All Environments' || attrs.environment === activeEnvironment;
      return catMatch && typeMatch && envMatch;
    });
  }, [sportsProducts, activeCategory, activeType, activeEnvironment]);

  // Products for "Essential Loadout" section
  const essential1 = sportsProducts.find(p => p.id === 's-3'); // Merino base layer
  const essential2 = sportsProducts.find(p => p.id === 's-5'); // Hiking pack
  const essential3 = sportsProducts.find(p => p.id === 's-7'); // Trail shoes
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-foreground selection:text-background font-sans overflow-x-hidden">
      ...
      <section className="relative min-h-[80vh] w-full pt-32 pb-16 px-4 md:px-8 flex items-center justify-center overflow-hidden">
        ...
        >
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tighter mb-6 text-foreground leading-[1.1]">
            Engineered for<br />Extremes.
          </h1>
          <p className="text-xl md:text-2xl text-foreground font-medium mb-8 max-w-2xl mx-auto drop-shadow-sm">
            High-performance gear designed to withstand the harshest environments and push human limits. No compromises.
          </p>
        </motion.div>
      </section>
      ...
      <section className="py-24 px-4 md:px-8 max-w-7xl mx-auto w-full bg-background" id="collection">
        ...
        <div className="mb-16">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-10">Shop by Activity.</h2>
          ...
          {/* Secondary Filters: Type & Environment */}
          <div className="flex flex-wrap items-center gap-4 border-t border-border pt-6">
            ...
            {(activeCategory !== 'All Activities' || activeType !== 'All Gear Types' || activeEnvironment !== 'All Environments') && (
              <button 
                onClick={() => { setActiveCategory('All Activities'); setActiveType('All Gear Types'); setActiveEnvironment('All Environments'); }}
      ...
      <section className="py-32 bg-muted/30 border-t border-border/50">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4">The Essential Loadout.</h2>
            <p className="text-text-muted text-lg max-w-2xl mx-auto">
              A meticulously curated core kit for any alpine adventure or high-output trail day.
            </p>
          </div>
